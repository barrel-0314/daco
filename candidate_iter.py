...
def og_distance(t1,t2):
    ...
    f=open('ontology_data/ontology_graph_undirect.data','rb')
    ogud=pickle.load(f)
    f.close()
...
def calculate_closeness(ctf,q):
    f=open('ontology_data/type_depth.data','rb')
    td=pickle.load(f)
    f.close()
    #f=open('../column_correction/type_freq.data','rb')
    #tf=pickle.load(f)
    #f.close()
    f=open('ontology_data/type_ic.data','rb')
    tic=pickle.load(f)
    f.close()
    f=open('ontology_data/rel_ic.data','rb')
    ric=pickle.load(f)
    f.close()
    
    f=open('ontology_data/pt.data','rb')
    pt=pickle.load(f)
    f.close()
    ...
    for cp in Close:
        ...
        if cp in CR and cp[0]!=cp[1]:
            ...
            if MR==0:
                ...
                CloseM[cp]=theta*Close[cp]+(1-theta)*CloseR[cp]*(MC/MR)
