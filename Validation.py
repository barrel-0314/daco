...
import read_data
import numpy as np
import candidate_generate_norm
from class_candidate import Candidate
import candidate_iter
import Relation_Discover_norm
...
def valid_candidate_initial(fpath,ns,vld_row,who_con,wt_name,db,tree_dict,word_dict):
    ...
    for r in range(len(vld_row)):
        for c in range(ns):
            ...
            if l not in C:
                ...
                result=candidate_generate_norm.topk_candidate_search(C[l],5,l)
    ...
    #print(Canl)
    C_type,C_rout,C_rin=candidate_generate_norm.candidate_type(Canl,tree_dict,word_dict)
    ...
    for pos in Can:
        ...
        if len(Can[pos])!=0:
            ...
            if len(l)<5:
                ...
                n=5
            if len(l)!=0:
                result,c_rel=candidate_generate_norm.candidate_search_in_literal(l,5,n,tree_dict,word_dict)
            else:
                result=[]
            for r in result:
...
def valid_relation_score(Vs,tree_dict,word_dict,ns):
    ...
    for j in range(ns):
        ...
        NS=NS.reshape(-1,1)
        NS[j]=1
        Rel,RR,S,O,R=Relation_Discover_norm.relation_discovery(Vs,NS,Rel,RR,tree_dict,word_dict)
...
def valid_main(fpath,wt_name,WT,Cs,ctf,vld_num,td,wd,db,cc):
    
    vld_flag=1
    
    vld_row=valid_sample(WT,Cs,vld_num)  
    Vs,Vw=valid_candidate_initial(fpath,WT.number_start,vld_row,WT.content,wt_name,db,td,wd)
